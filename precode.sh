...
ls dir2 -p | grep -v / > dir2/list.txt # -p | grep -v / отсеиваем директории, оставляя только файлы (в примере нет)
#find dir2 -maxdepth 1 -type f -printf '%f\n' > dir2/list.txt - второй способ решить данную задачу. -printf '%f\n' - убирает путь к файлам
...
dir2/hello.sh "$NAME" >> dir1/summary.txt # обязательно нужны двойные ковычки, что бы пременная передалась целиком
